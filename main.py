...
async def choose_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton("однаразавая", callback_data="2_1"),
...
async def get_notification_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f'ваше напоминание сохранино: {update.message.text} спасибо.\n теперь напишите время отправки напоминания')
    return NOTIFICATION_SEND_TIME
...
async def notifaction_send_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['name'] = update.message.text
    await update.message.reply_text(f'ваше напоминание будет сохранино {update.message.text}')
    keyboard = [
        [
            InlineKeyboardButton("Начнем", callback_data="1"),
            ],
